...
template<class UnschedCoreComputerT>
class GeneralFocusRefinement
{
public:
	typedef UnschedCoreComputerT UnschedCoreComputer;
	typedef typename UnschedCoreComputer::UnschedCore UnschedCore;
	typedef typename UnschedCoreComputer::SchedConds SchedConds;
	typedef typename UnschedCoreComputer::SchedCondElement SchedCondElement;
	typedef typename UnschedCoreComputer::System SystemType;
	typedef typename UnschedCoreComputer::UniqueSchedCondComp UniqueSchedCondComp;
	typedef deque<UnschedCore> UnschedCores;
	typedef map<SchedCondElement, IloNumVar, UniqueSchedCondComp> SchedCondVars;	
	enum IterationState
	{
		Search,
		Optimal,
...
public:
	...
	GeneralFocusRefinement(SystemType & rcTaskSet)
	...
	int FocusRefinement(int iDisplay, double dTimeout = 1e74)
	{
		...
		while (1)
		{
			...
			if (m_iBestSolFeasible == 1)
			{
				...
				m_cPriorityAssignmentSol = TaskSetPriorityStruct(*m_pcTaskSet);
	...
	TaskSetPriorityStruct getSolPA()	
	{
		return m_cPriorityAssignmentSol; 
	}

	void GenerateStatisticFile(char axFileName[])
	{
	...
	bool IsSchedulable(SchedConds & rcSchedCondsFlex, SchedConds & rcSchedCondsFixed, TaskSetPriorityStruct & rcPriorityAssignment)
	{
		return m_cUnschedCoreComputer.IsSchedulable(rcSchedCondsFlex, rcSchedCondsFixed, rcPriorityAssignment);
	}

	int ComputeUnschedCores(SchedConds & rcSchedCondsFlex, SchedConds & rcSchedCondsFixed, UnschedCores & rcUnschedCores, int iLimit)
	{
	...
	TaskSetPriorityStruct & getPriorityAssignmentSol()	{ return m_cPriorityAssignmentSol; }
	
	UnschedCoreComputer & getUnschedCoreComputer()	{ return m_cUnschedCoreComputer; }

	SchedConds getOptimalSchedCondConfig()	{ return m_dequeSolutionSchedConds.front().second; }

	void AddFixedSchedCond(SchedCondElement cFixedSchedCond)
	{
...
protected:	
	...
	void CreateSchedCondVars(const IloEnv & rcEnv, UnschedCores & rcUnschedCores, SchedCondVars & rcSchedCondVars)
	{
		for (typename UnschedCores::iterator iterCore = rcUnschedCores.begin(); iterCore != rcUnschedCores.end(); iterCore++)
		{
			...
			{
				rcSchedCondVars[*iterEle] = IloNumVar(rcEnv, 0.0, 1.0, IloNumVar::Int);
			}
		}
	}
	...
	void GenUnschedCoreConst(const IloEnv & rcEnv, UnschedCores & rcUnschedCores, SchedCondVars & rcSchedCondVars, IloRangeArray & rcConst)
	{
		for (typename UnschedCores::iterator iterCore = rcUnschedCores.begin(); iterCore != rcUnschedCores.end(); iterCore++)
		{
			...
			for (typename UnschedCore::iterator iterEle = iterCore->begin(); iterEle != iterCore->end(); iterEle++)
			{
				assert(rcSchedCondVars.count(*iterEle));
				cExpr += rcSchedCondVars[*iterEle];
			}
			rcConst.add(IloRange(rcEnv, 0.0, cExpr, iterCore->size() - 1));
		}
	}
	...
	virtual void ExtractSolution(IloCplex & rcSolver, int iSolIndex, SchedConds & rcSchedCondsConfig)
	{
	...
	void PrintIterationStatusSimple(IterationStatus & rcStatus)
	{
		...
		cout <<
			...
			" | t: " << rcStatus.dTime << "s   ";
#else
		printf_s("\rIt: %d | UB: %.2e | LB: %.2f | obj: %.2f | UC: %d | %s | t: %.2fs      ",
...
template<class UnschedCoreComputer_MK_T>
class GeneralFocusRefinement_MK
{
public:
	typedef UnschedCoreComputer_MK_T UnschedCoreComputer_MK;
	typedef typename UnschedCoreComputer_MK::UnschedCore UnschedCore;
	typedef typename UnschedCoreComputer_MK::SchedConds SchedConds;
	typedef typename UnschedCoreComputer_MK::SchedCond SchedCond;
	typedef typename UnschedCoreComputer_MK::SchedCondType SchedCondType;
	typedef typename UnschedCoreComputer_MK::SchedCondContent SchedCondContent;
	typedef typename UnschedCoreComputer_MK::SystemType SystemType;	
	typedef deque<UnschedCore> UnschedCores;	
...
protected:
	SystemType * m_pcTaskSet;
	UnschedCoreComputer_MK m_cUnschedCoreComputer;
...
public:
	...
	GeneralFocusRefinement_MK(SystemType & rcTaskSet)
		:m_cUnschedCoreComputer(rcTaskSet)
	{
		m_pcTaskSet = &rcTaskSet;
		GenerateFixedSchedConds(m_cSchedCondsFixed);
	...
	int BinaryFocusRefinement(double dTolerance, int iDisplay, double dObjLB = -1e74, double dObjUB = 1e74, double dTimeout = 1e74)
	{
		assert(m_pcTaskSet);
		GET_TASKSET_NUM_PTR(m_pcTaskSet, iTaskNum, pcTaskArray);
		m_cSchedCondsConfig.clear();
		...
		double dLastObjective = -1e74;
		int iIterationCount = 0;
		int iBinaryIterationCount = 0;
		double dUCComputeTime = 0;

		while (dObjUB - dObjLB > dTolerance)
		{
			bool bCanExit = false;
			iStatus = 0;
			double dMiddle = (dObjLB + dObjUB) / 2;
			...
			while (1)
			{
				bCanExit = false;
				if (iDisplay == 2)	cout << "ILP query...." << endl;
				iStatus = SolveBinaryFR(m_iSubILPDisplay, m_cSchedCondsFixed, m_cUnschedCores, dObjLB, dMiddle, 1e74);
				dLastObjective = m_dObjective;
				double dUCSeconds = getWallTimeSecond();
				UnschedCores cThisCores;
				if (iDisplay == 2) cout << "Unched-core computation..." << endl;
				...
				if (iDisplay)
				{
					PrintIteratationStatusSimple_BinaryFR(m_dequeIterationLogs.back(), iBinaryIterationCount, dObjLB, dObjUB);
					//system("pause");
				}

				PrintIterStatusToLog_BinaryFR(m_dequeIterationLogs.size() - 1, iBinaryIterationCount, dObjLB, dObjUB);
				if (bCanExit)
				{					
					break;
				}
			...
			BinaryFR_RefineBound(dObjLB, dObjUB, iStatus);
			iBinaryIterationCount++;
		}
		if (iDisplay) cout << endl;		
		ComputeStatistic();
		if (iStatus == 1)
	...
	virtual UnschedCoreComputer_MK & getUnschedCoreComputer()	{ return m_cUnschedCoreComputer; }
...
protected:
	...
	virtual int SolveBinaryFR(int iDisplay, SchedConds & rcSchedCondFixed, UnschedCores & rcGenUC, double dObjLB, double dObjUB, double dTimeout = 1e74)
	{
		try
		{
			IloEnv cSolveEnv;
	...
	{
		printf_s("\rBinStat: %d, [%.2e, %.2e] | It: %d | UB: %.2e | LB: %.2e | obj: %.2e | UC: %d | %s | t: %.2fs      ",
...
protected:
	...
	void GenRefSchedCondsModel(const IloEnv & rcEnv, SchedConds & rcRefSchedConds, SchedCondTypeVars & rcSchedCondTypeVars, IloNumVarArray & rcAbsDiffVars, IloRangeArray & rcConst, IloObjective & rcObj)
	{
	...
	void GenRefGuidedSearchModel(const IloEnv & rcEnv, SchedConds & rcRefSchedConds, SchedCondTypeVars & rcSchedCondTypeVars, IloModel & rcModel)
	{
	...
	void GenReverseRefGuidedSearchModel(const IloEnv & rcEnv, SchedConds & rcRefSchedConds, SchedCondTypeVars & rcSchedCondTypeVars, IloModel & rcModel)
	{
		IloNumVarArray rcAbsDiffVars(rcEnv);
		IloRangeArray rcConst(rcEnv);
		IloObjective rcObj(rcEnv);
	...
	void GenFixedSchedCondsConst(SchedConds & rcFixedSchedConds, SchedCondTypeVars & rcSchedCondTypeVars, IloRangeArray & rcConst)
	{
