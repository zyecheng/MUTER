...
template<class SchedCondType, class UniqueSchedCondCompT = less<SchedCondType>, class SystemType = TaskSet>
class GeneralUnschedCoreComputer
{
public:
	typedef SchedCondType SchedCondElement;
	typedef MySet<SchedCondType> SchedConds;	
	typedef vector<SchedCondType> VectorofSchedConds;
	typedef deque<SchedConds> SchedCondsDeque;
	typedef map<SchedCondType, int> SchedCond2Occurence;
	typedef SchedConds UnschedCore;
	typedef SchedCondsDeque UnschedCores;
	typedef SystemType System;
	typedef UniqueSchedCondCompT UniqueSchedCondComp;

	class SchedCondsComparator
	{
	public:
		bool operator () (const SchedConds & rcLHS, const SchedConds & rcRHS)
		{
			int iSizeRHS = rcRHS.size();
			int iSizeLHS = rcLHS.size();
			if (iSizeLHS == iSizeRHS)
			{
	...
	class SchedCondOccurenceComp
	{
	public:
		SchedCond2Occurence * m_pcOccurenceData;

		SchedCondOccurenceComp()	{}

		SchedCondOccurenceComp(SchedCond2Occurence & rcOccurenceData)
		{
			m_pcOccurenceData = &rcOccurenceData;
		}

		bool operator()(const SchedCondType & rcLHS, const SchedCondType & rcRHS)
		{
			int iOccurLHS = (*m_pcOccurenceData)[rcLHS];
			int iOccurRHS = (*m_pcOccurenceData)[rcRHS];
	...
	typedef MySet<SchedCondType, SchedCondOccurenceComp> SchedCondsOccurSort;

protected:
	SystemType * m_pcTaskSet;
	SchedCond2Occurence m_mapOccurence;
public:
	GeneralUnschedCoreComputer()
	{

	}

	GeneralUnschedCoreComputer(SystemType & rcSystem)
	{
	...
	virtual bool IsSchedulable(SchedConds & rcSchedCondsFlex, SchedConds & rcSchedCondsFixed, TaskSetPriorityStruct & rcPriorityAssignment)
	{
		...
		for (; iPriority >= 0; iPriority--)
		{
			...
			for (int i = 0; i < iTaskNum; i++)
			{
				...
				PreSchedTest(iCand, rcPriorityAssignment, rcSchedCondsFlex, rcSchedCondsFixed);
				bool bSchedStatus = SchedTest(iCand, cPriorityAssignment, rcSchedCondsFlex, rcSchedCondsFixed);
				PostSchedTest(iCand, cPriorityAssignment, bSchedStatus, rcSchedCondsFlex, rcSchedCondsFixed);
				if (bSchedStatus)
				{
					bAssigned = true;
					break;
	...
	virtual void PreComputeUnschedCores(SchedConds & rcSchedCondsFlex, SchedConds & rcSchedConstFixed, SchedCondsDeque & rcUnschedCores, int iLimit, double dTimeout = 1e74)
	{
		for (typename SchedConds::iterator iter = rcSchedCondsFlex.begin(); iter != rcSchedCondsFlex.end(); iter++)
		{
			m_mapOccurence[*iter] = 0;
		}
	}

	int ComputeUnschedCores(SchedConds & rcSchedCondsFlex, SchedConds & rcSchedConstFixed, SchedCondsDeque & rcUnschedCores, int iLimit, double dTimeout = 1e74)
	{
		assert(m_pcTaskSet);
	...
	virtual int ComputeAllCores(SchedConds & rcSchedConst, SchedCondsDeque & rcUnschedCores, int iLimit, double dTimeout = 1e74)
	{
		return 0;
	}
	
	int CompareSchedConds(const SchedConds & rcLHS, const SchedConds & rcRHS)
	{
		SchedCondsComparator cComp;
		if (cComp(rcLHS, rcRHS))
		{
			return 1;
		}
		else if (cComp(rcRHS, rcLHS))
		{
			return -1;
		}
		return 0;
...
protected:
	...
	virtual bool PreSchedTest(int iTaskIndex, TaskSetPriorityStruct & rcPriorityAssignment, SchedConds & rcSchedCondsFlex, SchedConds & rcSchedCondsFixed)
	{
		return true;
	}

	virtual bool PostSchedTest(int iTaskIndex, TaskSetPriorityStruct & rcPriorityAssignment, bool bSchedStatus, SchedConds & rcSchedCondsFlex, SchedConds & rcSchedCondsFixed)
	{
		return true;
	}	

	virtual void ConvertToMUS_Schedulable(SchedCondType & rcErased, SchedConds & rcSchedCondsFlex, SchedConds & rcShcedCondsFixed, TaskSetPriorityStruct & rcPriorityAssignment)
	{
	...
	{			
		...
		vector<SchedCondType> vectorSorted;
		...
		TaskSetPriorityStruct cPriorityAssignmentLastUnsched(*m_pcTaskSet);
		for (typename vector<SchedCondType>::iterator iter = vectorSorted.begin(); iter != vectorSorted.end(); iter++)
		{						
			rcCore.erase(*iter);			
			bool bSchedulable = IsSchedulable(rcCore, rcSchedCondsFixed, cPriorityAssignment);
			if (bSchedulable)
			{				
				ConvertToMUS_Schedulable(*iter, rcCore, rcSchedCondsFixed, cPriorityAssignmentLastUnsched);
	...
	virtual void MUSConvertOrder(SchedConds & rcSchedCondsFlex, vector<SchedCondType> & rvectorSchedConds)
	{		
		MUSConvertOrderByOccurence(rcSchedCondsFlex, rvectorSchedConds);
	}

	void MUSConvertOrderByOccurence(SchedConds & rcSchedCondsFlex, vector<SchedCondType> & rvectorSchedConds)
	{
		SchedCondOccurenceComp cComp(m_mapOccurence);
		SchedCondsOccurSort cOccurSort(cComp);
		for (typename SchedConds::iterator iter = rcSchedCondsFlex.begin(); iter != rcSchedCondsFlex.end(); iter++)
		{			
			cOccurSort.insert(*iter);
		}

		for (typename SchedCondsOccurSort::iterator iter = cOccurSort.begin(); iter != cOccurSort.end(); iter++)
		{
	...
	virtual void UpdateOccurStat(SchedConds & rcCore)
	{
		for (typename SchedConds::iterator iter = rcCore.begin(); iter != rcCore.end(); iter++)
		{
			if (m_mapOccurence.count(*iter))
			{
				m_mapOccurence[*iter]++;
			}
			else
			{
				m_mapOccurence[*iter] = 1;
			}
		}
	}

	int Recur(int iLevel, int iLimit, SchedConds & rcSchedCondsFlex,
		SchedConds & rcSchedCondsFixed, SchedCondsDeque & rdequeCollectedCores, double dTimeStart, double dTimeout)
	{
		...
		if (bNewMiniSet || rdequeCollectedCores.size() > iLevel)
		{
			...
			if (!IsCoreContained(rcSchedCondsFlex, rcMUS))
				return -1;
			for (typename SchedConds::iterator iter = rcMUS.begin(); iter != rcMUS.end(); iter++)
			{								
				assert(rcSchedCondsFlex.count(*iter));
				SchedCondType iSchedCondEle = *rcSchedCondsFlex.find(*iter);
				NextCore_PerturbSchedCondEle(iSchedCondEle, *iter, rcSchedCondsFlex);
				...
				rcSchedCondsFlex.insert(iSchedCondEle);
				NextCore_RestoreSchedCondEle(iSchedCondEle, *iter, rcSchedCondsFlex);
			}
		}
		return 0;
	}
...
template<class SchedCondKey, class SchedCondDat, class System>
class GeneralUnschedCoreComputer_MK
{
public:
	typedef SchedCondKey SchedCondType;
	typedef SchedCondDat SchedCondContent;
...
protected:
	SystemType * m_pcTaskSet;
public:
	GeneralUnschedCoreComputer_MK()
	{

	}

	GeneralUnschedCoreComputer_MK(SystemType & rcSystem)
	{
		m_pcTaskSet = &rcSystem;
	}
...
protected:
	virtual bool IsCoreContained(SchedConds & rcSchedConds, UnschedCore & rcCore)
	{
		if (rcSchedConds.size() < rcCore.size())
			return false;		
		typename SchedConds::iterator iterCore = rcCore.begin();
		for (; iterCore != rcCore.end(); iterCore++)
		{
			if (rcSchedConds.find(iterCore->first) == rcSchedConds.end())
			{
				return false;
			}
		}
	...
	virtual int PostSchedTest(int iTaskIndex, TaskSetPriorityStruct & rcPriorityAssignment, bool bSchedStatus, SchedConds & rcSchedCondsFlex, SchedConds & rcSchedCondsFixed)
	{
		return 1;
	}	
	...
	virtual bool IsRedundantSchedCond(SchedCond & rcSchedCond)
	{
		return false;
	}
		...
		SchedConds & rcSchedCondsFixed, UnschedCores & rdequeCollectedCores, double dTimeStart, double dTimeout)
	{
		...
		if (bNewMiniSet || rdequeCollectedCores.size() > iLevel)
		{
			...
			for (typename SchedConds::iterator iter = rcMUS.begin(); iter != rcMUS.end(); iter++)
			{
				...
				Recur_MinimalRelaxation(rcSchedCondsFlex, cSchedCondEle, cSchedCondEleMUS);
				int iNextLevel = iLevel + 1;
				int iStatus = Recur(iNextLevel, iLimit, rcSchedCondsFlex, rcSchedCondsFixed, rdequeCollectedCores, dTimeStart, dTimeout);
				while (iStatus == -1)
				{
...
template<class System, class SchedCondKey, class SchedCondData = __int64>
...
protected:
	...
	bool AudsleyPartialOrderBurden(int iTaskIndex, TaskSetPriorityStruct & rcPriorityAssignment, IntArray2D & rcEnforcedPartialOrder)
	{
		int iTaskPriority = rcPriorityAssignment.getPriorityByTask(iTaskIndex);
		GET_TASKSET_NUM_PTR(m_pcTaskSet, iTaskNum, pcTaskArray);
		for (int i = 0; i < iTaskNum; i++)
		{
			if ((m_cEnforcedPartialOrder[iTaskIndex][i] == 1) && (rcPriorityAssignment.getPriorityByTask(i) == -1))
			{
				return false;
			}
		}
